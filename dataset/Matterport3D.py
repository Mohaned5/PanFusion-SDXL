...
class Mp3dDataset(PanoDataset):
    def load_split(self, mode):
        if self.config['load_layout']:
            ...
            # Use filtered split files when load_layout is False
            split_file = 'valid_train.npy' if mode == 'train' else 'valid_test.npy'
...
class Matterport3D(PanoDataModule):
    ...
    def scan_pano_images(self):
        """Scan and filter panorama images when load_layout is False."""
        from PIL import Image
        import numpy as np
        
        splits = ['train', 'test']  # 'val' uses 'test.npy' in current setup
        for split in splits:
            src_split_file = os.path.join(self.hparams.data_dir, f"{split}.npy")
            dst_split_file = os.path.join(self.hparams.data_dir, f"valid_{split}.npy")
            if os.path.exists(dst_split_file):
                print(f"Using existing {dst_split_file}")
                continue

            print(f"Scanning {split} split for valid panorama images...")
            data = np.load(src_split_file)
            valid_data = []
            for d in data:
                scene_id, _, view_id = d[0].split('/')
                view_id = view_id.split('_')[0]
                pano_path = os.path.join(self.hparams.data_dir, scene_id, 'matterport_stitched_images', f"{view_id}.png")
                try:
                    with Image.open(pano_path) as img:
                        img.verify()  # Check if the image is valid
                    valid_data.append(d)
                except Exception as e:
                    print(f"Skipping invalid image: {pano_path}, error: {e}")
            valid_data = np.array(valid_data)
            np.save(dst_split_file, valid_data)
            print(f"Saved {len(valid_data)}/{len(data)} valid samples to {dst_split_file}")

    def prepare_data(self):
        if os.path.isdir(self.hparams.data_dir):
            if self.hparams.load_layout:
                self.scan_layout_cond()
            else:
                self.scan_pano_images()